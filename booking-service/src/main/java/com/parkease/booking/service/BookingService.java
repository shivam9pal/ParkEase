...
public interface BookingService {
     ...
     *
     * PRE_BOOKING → status RESERVED, spot RESERVED, checkInTime null
     * WALK_IN     → status ACTIVE, spot OCCUPIED, checkInTime = now
     *
    ...
    /**
     * Transitions a PRE_BOOKING from RESERVED → ACTIVE.
     * Calls spot-service occupySpot(). Sets checkInTime = now.
     *
     ...
     * Calculates fare based on actual checkInTime → now (min 1 hour).
     * Releases spot, increments lot counter, persists totalAmount.
     *
    ...
    /**
     * Cancels a booking in RESERVED or ACTIVE status.
     * Releases spot, increments lot counter.
     *
     * @param requesterRole  "DRIVER", "MANAGER", or "ADMIN" — controls access scope
     * @throws IllegalStateException  if status is COMPLETED or already CANCELLED
     * @throws SecurityException      if DRIVER tries to cancel another user's booking
     */
    BookingResponse cancelBooking(UUID bookingId, UUID userId, String requesterRole);

    /**
     * Extends the planned endTime of a RESERVED or ACTIVE booking.
     * No spot changes — spot is already held. Fare still calculated at checkOut.
     *
     ...
     * Caller's authorization (owner or MANAGER/ADMIN) enforced in controller.
     *
    ...
    /**
     * All bookings (any status) for a given parking lot.
     * Used by MANAGER/ADMIN dashboard.
     */
     ...
     * Controller enforces ADMIN role before calling this.
     */
    ...
    /**
     * Real-time fare estimate for an ACTIVE booking.
     * Uses booking.pricePerHour (snapshotted at creation) × elapsed hours.
     * Minimum 1 hour enforced.
     *
     * Does NOT persist anything — read-only estimate.
     *
     * @throws IllegalStateException  if booking is not ACTIVE (must be checked in)
     */
    ...
    /**
     * Auto-cancels expired PRE_BOOKING reservations.
     * Called by BookingExpiryScheduler every 5 minutes.
     *
     * Finds all RESERVED PRE_BOOKINGs where startTime < (now - gracePeriodMinutes).
     * For each: releases spot → increments lot → sets CANCELLED → publishes event.
     *
     * This method is intentionally package-accessible to the scheduler only.
     * No JWT context — runs as system operation.
     */
